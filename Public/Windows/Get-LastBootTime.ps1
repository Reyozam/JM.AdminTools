...
function Get-LastBootTime
{
    ...
    Process{
        ...
        {
            if($LocalAddress -contains $Computer)
            {			
            ...
            else
            {
                if(Test-Connection -ComputerName $Computer -Count 2 -Quiet)
                {
                    try {
                        ...
                        {
                            $LastBootTime = Invoke-Command -ScriptBlock $Scriptblock -ComputerName $Computer -ArgumentList $Search -Credential $Credential -ErrorAction Stop
                        }
                        else
                        {					    
                            $LastBootTime = Invoke-Command -ScriptBlock $Scriptblock -ComputerName $Computer -ArgumentList $Search -ErrorAction Stop
                        }
                    }
                    catch {
                ...
                {				
                    Write-Error -Message """$Computer"" is not reachable via ICMP!" -Category ConnectionError
